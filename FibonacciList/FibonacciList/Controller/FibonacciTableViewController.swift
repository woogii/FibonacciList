...
extension FibonacciTableViewController {
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let cell = tableView.dequeueReusableCell(withIdentifier: Constant.CellIDs.FibonacciCell, for: indexPath) as! FibonacciTableViewCell
  ...
  private func configureCell(cell:FibonacciTableViewCell, indexPath:IndexPath) {
    guard let fibonacciNumber = fibonacciList.numbersDict[indexPath.row] else {
      return
    }
    cell.fibonacciNumberLabel.text = Constant.FibonacciInfoPrefix + " \(indexPath.row) : \(fibonacciNumber.intValue)"
  }
